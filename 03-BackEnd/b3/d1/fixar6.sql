...
-- Os resultados devem ser agrupados pela classificação indicativa e 
-- ordenados da maior média para a menor.

-- rating, AVG(replacement_cost)
-- FROM sakila.film
-- GROUP BY rating
-- ORDER BY rating, replecement_cost DESC ;
...
-- Monte uma query para a tabela sakila.address que exiba o nome do distrito e 
-- a quantidade de endereços registrados nele. Os resultados devem ser ordenados 
-- da maior quantidade para a menor.

SELECT district, COUNT(address) AS 'enderecos'
FROM sakila.address
GROUP BY district
...
SELECT rating, SUM(replacement_cost) AS soma_substituicao
FROM sakila.film
GROUP by rating
HAVING soma_substituicao > 3950.5
ORDER BY soma_substituicao ASC;
